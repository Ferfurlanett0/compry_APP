...
import '../../../shopping_lists/domain/repositories/shopping_list_repository.dart';
...
final historyProvider =
  ...
  final listsStream = _watchListsForUser(repository, currentUser);
  return listsStream.map(
    (lists) => lists.where((list) => list.status.isFinished).toList(),
  );
});

Stream<List<ShoppingListEntity>> _watchListsForUser(
  ShoppingListRepository repository,
  UserEntity? currentUser,
) {
  if (currentUser?.isAdmin == true) {
    return repository.watchAllLists();
  }

  final userId = currentUser?.id;
  if (userId == null) {
    return Stream.value(const <ShoppingListEntity>[]);
  }

  return repository.watchEmployeeLists(userId);
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: historyAsync.when(
        loading: () =>
            const SingleChildScrollView(child: ListCardSkeletonList()),
